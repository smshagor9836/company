    						...
    						<div class="row">
    							<div class="col-lg-9">
    								<div class="media-list model-media-list">
                                          ...
                                          {{--  @php
                                           $editalbleImages= isset($img_array)? $img_array : [];
                                           
                                           @endphp
                                            @foreach($editalbleImages as $key=>$media)
                                                <div class="col-lg-2 media-hover">
                                                <input type="checkbox" class="checkbox" name="multi[]" value="{{ $media }}" id="me1{{ $key }}" checked="">
                                                <label class="single-img active"  for="me1{{  $key  }}" id="media-level1{{ $key }}" onclick="active1({{ $key }})">

                                                    <div class="single-media model-media text-center">
                                                        <img class="img-fluid" src="{{ asset($media) }}" alt="{{ $media }}">  
                                                    </div>
                                                </label>
                                            </div>  
                                            @endforeach --}}
