...
<div class="card"  >
	<div class="card-body">
		<div class="row mb-30">
			<div class="col-lg-6">
				<h4>{{ __('Service List') }}</h4>
			</div>
			<div class="col-lg-6">
				<div class="add-new-btn">
					<a href="{{ route('admin.service.service.create') }}" class="btn f-right">{{ __('Add New') }}</a>
				</div>
			</div>
		</div>
		<div class="cart-filter mb-20">
			<a href="{{ route('admin.service.service.index') }}">{{ __('All') }} <span>({{ App\Terms::where('type',4)->where('status','!=',0)->count() }})</span></a>
			<a href="?st=1">{{ __('Published') }} <span>({{ App\Terms::where('type',4)->where('status',1)->count() }})</span></a>
			<a href="?st=2">{{ __('Drafts') }} <span>({{ App\Terms::where('type',4)->where('status',2)->count() }})</span></a>
			<a href="?st=trash" class="trash">{{ __('Trash') }} <span>({{ App\Terms::where('type',4)->where('status',0)->count() }})</span></a>
		</div>
		<div class="card-action-filter">
			<form method="post" id="basicform" action="{{ route('admin.service.services.destroy') }}">
				@csrf
				<div class="row">
					<div class="col-lg-6">
						<div class="d-flex">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{{ $post->title }}
								<div class="hover">
									<a href="{{ route('admin.service.service.edit',$post->id) }}">{{ __('Edit') }}</a>

									<a href="{{ route('admin.service.service.edit',$post->id) }}" class="last">{{ __('View') }}</a>
								</div>
							</td>
							<td>
								{{ route('service.show',$post->slug)  }}
							</td>
