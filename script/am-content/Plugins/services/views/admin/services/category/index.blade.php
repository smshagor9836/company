...
<div class="row" id="category_body">
	<div class="col-lg-4">      
		<div class="card">
			<div class="card-body">
				<form id="basicform" method="post" action="{{ route('admin.service.category.store') }}">
					@csrf
	...
	<div class="col-lg-8" >      
		<div class="card">
			<div class="card-body">
				@php
				if (!empty($req)) {
					$categeories=App\Category::where('name','LIKE','%'.$req.'%' )->where('type',3)->latest()->paginate(15);	
				}
				else{
					$categeories=App\Category::where('type',3)->latest()->paginate(15);
				}
				@endphp
				<div class="table-responsive">
					<div class="card-action-filter">
						<form id="basicform1" method="post" action="{{ route('admin.service.categorys.destroy') }}">
							@csrf
							...
							<tbody>
								...
								<tr>
									...
									<td>
										{{ $category->name }}
										<div class="hover">
											<a href="{{ route('admin.service.category.edit',$category->id) }}">{{ __('Edit') }}</a>
										</div>
									</td>
									<td>{{ $category->slug }}</td>
