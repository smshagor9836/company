...
<div class="card"  >
	<div class="card-body">
		<div class="row mb-30">
			<div class="col-lg-6">
				<h4>{{ __('Pricing Table List') }}</h4>
			</div>
			<div class="col-lg-6">
				<div class="add-new-btn">
					<a href="{{ route('admin.ecommerce.pricing.create') }}" class="btn f-right">{{ __('Add New') }}</a>
				</div>
			</div>
		</div>
		<div class="cart-filter mb-20">
			<a href="{{ route('admin.ecommerce.pricing.index') }}">{{ __('All') }} <span>({{ App\Terms::where('type',2)->where('status','!=',0)->count() }})</span></a>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{{ $post->title }}
								<div class="hover">
									<a href="{{ route('admin.ecommerce.pricing.edit',$post->id) }}">{{ __('Edit') }}</a>

									<a href="{{ route('admin.ecommerce.pricing.edit',$post->id) }}" class="last">{{ __('View') }}</a>
								</div>
							</td>
							<td>
