...
<div class="row" id="category_body">
	<div class="col-lg-4">      
		<div class="card">
			<div class="card-body">
				<form id="basicform" method="post" action="{{ route('admin.ecommerce.category.store') }}">
					@csrf
						...
						<div class="form-group">
							...
							<select class="custom-select mr-sm-2" name="p_id" id="p_id inlineFormCustomSelect">
								<option value="">None</option>
								  <?php echo ConfigCategory(2) ?>
								
							</select>
						</div>
						<div class="form-group mt-20">
							<button class="btn col-12" type="submit">{{ __('Add New Category') }}</button>
						</div>
					</div>
				</form>
			</div>
	...
	<div class="col-lg-8" >      
		<div class="card">
			<div class="card-body">
				@php
				if (!empty($req)) {
					$categeories=App\Category::where('name','LIKE','%'.$req.'%' )->where('type',2)->latest()->paginate(15);	
				}
				else{
					$categeories=App\Category::where('type',2)->latest()->paginate(15);
				}
				@endphp
				<div class="table-responsive">
					<div class="card-action-filter">
						<form id="basicform1" method="post" action="{{ route('admin.ecommerce.categorys.destroy') }}">
							@csrf
							...
							<tbody>
								...
								<tr>
									...
									<td>
										{{ $category->name }}
										<div class="hover">
											<a href="{{ route('admin.ecommerce.category.edit',$category->id) }}">{{ __('Edit') }}</a>
										</div>
									</td>
									<td>{{ $category->slug }}</td>
					...
					<div style="float: right">{{ $categeories->links() }}</div>
				</div>
