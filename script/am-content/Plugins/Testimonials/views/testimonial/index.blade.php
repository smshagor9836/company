		...
		<div class="row mb-30">
			...
			<div class="col-lg-6">
				<div class="add-new-btn">
					<a href="{{ route('admin.testimonial.create') }}" class="btn f-right">{{ __('Add New') }}</a>
				</div>
			</div>
		</div>
		<div class="cart-filter mb-20">
			<a href="{{ route('admin.testimonial.index') }}">{{ __('All') }} <span>({{ App\Terms::where('type',5)->where('status','!=',0)->count() }})</span></a>
			<a href="?st=1">{{ __('Published') }} <span>({{ App\Terms::where('type',5)->where('status',1)->count() }})</span></a>
			<a href="?st=2">{{ __('Drafts') }} <span>({{ App\Terms::where('type',5)->where('status',2)->count() }})</span></a>
			<a href="?st=trash" class="trash">{{ __('Trash') }} <span>({{ App\Terms::where('type',5)->where('status',0)->count() }})</span></a>
		</div>
		<div class="card-action-filter">
			<form method="post" id="basicform" action="{{ route('admin.testimonials.destroy') }}">
				@csrf
				<div class="row">
					<div class="col-lg-6">
						<div class="d-flex">
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th class="am-title">{{ __('Client Name') }}</th>
							<th class="am-title">{{ __('Company Name') }}</th>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{{ $post->title }}
								<div>
									<a href="{{ route('admin.testimonial.edit',$post->id) }}">Edit</a>
								</div>
							</td>
